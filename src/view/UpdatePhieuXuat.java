...
public class UpdatePhieuXuat extends javax.swing.JDialog {
    ...
    public ChiTietPhieu findCTPhieu(String maMay) {
        for (var i : CTPhieu) {
            if (maMay.equals(i.getMayTinh().getMaMay())) {
                return i;
            }
        }
        return null;
        ...
        try {
            ...
            for (int i = 0; i < CTPhieu.size(); i++) {
                tblNhapHangmd.addRow(new Object[]{
                    i + 1, CTPhieu.get(i).getMayTinh().getMaMay(), findMayTinh(CTPhieu.get(i).getMayTinh().getMaMay()).getTenMay(), CTPhieu.get(i).getSoLuong(), formatter.format(CTPhieu.get(i).getDonGia()) + "đ"
                });
            }
        } catch (Exception e) {
        }
    ...
    private void btnNhapHangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNhapHangActionPerformed
        ...
        if (CTPhieu.isEmpty()) {
            ...
            for (var ct : CTPhieuOld) {
                MayTinhDAO.getInstance().updateSoLuong(ct.getMayTinh().getMaMay(), MayTinhDAO.getInstance().selectById(ct.getMayTinh().getMaMay()).getSoLuong() + ct.getSoLuong());
        ...
        if (i_row == -1) {
            ...
            if (newSL != null) {
                ...
                try {
                    ...
                    if (soLuong > findMayTinh(CTPhieu.get(i_row).getMayTinh().getMaMay()).getSoLuong()) {
            ...
            if (soluongselect == 0) {
                ...
                if (soluongselect < soluong) {
                    ...
                    if (mtl != null) {
                        ...
                        ChiTietPhieu ctp = new ChiTietPhieu(phieuxuat.getMaPhieu(), mt, soluong, mt.getGia());
                        CTPhieu.add(ctp);
                    }
                    loadDataToTableNhapHang();
                    textTongTien.setText(formatter.format(tinhTongTien()) + "đ");
